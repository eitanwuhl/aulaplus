...
import type { DashboardEvolucionData } from '@/types/schoolCatalog';
...
const DashboardEvolucion = ({ student }: DashboardEvolucionProps) => {
  const dashboard = student.dashboardEvolucion;

  if (!dashboard?.metricas) {
    return (
      <p className="text-sm text-muted-foreground text-center py-6">
        No hay métricas de evolución registradas para este alumno.
      </p>
    );
  }

  const { metricas } = dashboard;
  const deltaPromedio = metricas.promedioGeneral - metricas.promedioAnterior;
            ...
            <CardContent className="p-4">
              ...
              <div className="mt-2">
                <Badge variant="secondary" className="text-xs bg-blue-200 text-blue-800">
                  {deltaPromedio >= 0 ? '+' : ''}
                  {deltaPromedio.toFixed(1)} vs año anterior
                </Badge>
              </div>
            </CardContent>
          </Card>
